"""Native cognis-connect emit for redpath — forward findings to any platform.

Maps redpath's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    redpath ... --format json | redpath-emit --to stix
    redpath-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('target', '') + ' - ' + rec.get('source', '')
