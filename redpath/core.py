...
TOOL_NAME = "REDPATH"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'RP-PATH-001', 'severity': 'critical', 'weight': 3.0, 'title': 'SHORT_PATH_TO_DA', 'when': {'hops_to_da': '^[1-3]$'}, 'remediation': 'Tier-0 exposure. Break the path now (LAPS / kerberoast remediation / privilege removal).'}, {'id': 'RP-KERB-001', 'severity': 'high', 'weight': 2.5, 'title': 'KERBEROASTABLE_TIER0', 'when': {'kerberoastable': True, 'tier': 'tier0'}, 'remediation': 'Set strong password / move SPN / disable account.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="ad-attack-path",
                    metadata=item,
                ))
